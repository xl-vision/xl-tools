...
program
    ...
    .option('--src <path>', `The source files`, 'src/**/*.scss')
...
program
    ...
    .option('--testMatch <path>', `The source files`, '**/src/**/test/*.ts?(x)')
    .option('--setupFiles <path>', `The setup files`)
    ...
    .action(function (options) {
        const testMatch = splitToArray(options.testMatch)
        const setupFiles = splitToArray(options.setupFiles)
        const jestConfig = getConfig(options.jestConfig, jestConfigFile)
        const tsConfig = getConfig(options.tsConfig, tsConfigFile)
        return test({ ...parseOptions(options), testMatch, jestConfig, tsConfig, setupFiles })
    })
