...
const scsslint = require('./tasks/scsslint')
...
program
    ...
    .option('--src <path>', `The source files`, splitToArray)
    .option('--stylelintConfig <path>', `Specify stylelint config file, default is '${stylelintConfigFile}' if exists`)
    .action(function (options) {
        options = parseOptions(options)
        return scsslint(options)
    })
...
program
    ...
    .option('--updateSnapshot', `Update snapshot`)
    .action(function (options) {
