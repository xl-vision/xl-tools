...
md.use(MarkdownItContainer, 'demo', {
  ...
  render: function (tokens, idx) {
    ...
    if (tokens[idx].nesting === 1) {
      ...
      return `
      <DemoBox view={<${fnName}/>} code={\`${code.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`} title={<div>${title}</div>} desc={<div>${desc}</div>}>
      `
    }
    return '</DemoBox>'
  }
...
function renderModule (jsx, className, displayName) {
  const content = `
  ${prefixContent.join('\n')}
  ...
  MarkdownLoader.displayName = '${displayName}'
...
module.exports = function (source) {
  ...
  displayName = displayName || path.basename(this.resourcePath)
  ...
  // 生成组件
  return renderModule(content, className, displayName)
}
