...
md.renderer.rules.table_open = () => `<div class='${clsPrefix}_table-wrapper'><table class="${clsPrefix}_table">`
...
md.renderer.rules.code_inline = function (tokens, idx, options, env, self) {
  ...
  const ret = defaultCodeInlineRender(tokens, idx, options, env, self)
...
md.use(MarkdownItContainer, 'demo', {
  ...
  render: function (tokens, idx) {
    ...
    if (tokens[idx].nesting === 1) {
      ...
      let desc = ''
      let i = 1
      if (tokens[idx + i].type === 'paragraph_open') {
        desc = md.renderInline(tokens[idx + i + 1].content)
        // 删除多余的desc
        tokens[idx + i + 1].content = ''
        tokens[idx + i + 1].children = []
        i += 3
      }

      const code = tokens[idx + i].content

      let style = ''
      i++

      // 判断是否有样式
      if (tokens[idx + i].type === 'fence') {
        style = tokens[idx + i].content
      }

      const fnName = `Demo_${fnIndex++}`
      let formatStyle = style
      // 加入到前置内容中
      const formatCode = code.replace(/ *export +default/, ` const ${fnName} = `).replace(/className *= *["'](.*)["']/g, (match) => {
        const className = match.match(/className *= *["'](.*)["']/)[1]
        const classes = className.split(/ +/)
        // 替换style
        for (const cls of classes) {
          formatStyle = formatStyle.replace(new RegExp(cls, 'g'), `${fnName}_${cls}`)
        }
        return `className= "${classes.map(it => `${fnName}_${it}`).join(' ')}"`
      })

      prefixContent.push(formatCode)

      return `
      <style type='text/css' dangerouslySetInnerHTML = {{ __html: \`${formatStyle.replace(/`/g, '\\`')}\` }}/>
      <DemoBox view={<${fnName}/>} code={\`${code.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`} title={<div>${title}</div>} desc={<div>${desc}</div>} style={\`${style.replace(/`/g, '\\`')}\`}>
      `
    }
    return '</DemoBox>'
  }
...
function renderModule (jsx, className) {
  const content = `
  ${prefixContent.join('\n')}
...
module.exports = function (source) {
  ...
  // 生成组件
  return renderModule(content, className)
}
