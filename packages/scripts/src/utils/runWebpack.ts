...
function runWebpack(
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    if (devServerConfig) {
      ...
      getPort(HOST, defaultPort).then((port) => {
        if (port === null) {
          const error = 'Could not find a port.'
          console.error(chalk.red(error))
          return reject(error)
        }
        console.info(chalk.cyan(`You are now in dev server mode.`))
        console.info(
          chalk.cyan(`Starting the development server in port:${port}\n`)
        )
    ...
    compiler.run((err, stats) => {
      ...
      console.info(chalk.cyan(buildInfo))
      return resolve()
    })
  })
